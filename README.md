...
- **일일 중복 알림 방지**: 하루에 한 번 알림을 보낸 종목은 장이 끝날 때까지 다시 알림을 보내지 않습니다.
- **메시지 자동 분할**: 슬랙으로 보낼 종목이 많을 경우, 메시지를 여러 개로 자동으로 나누어 전송하여 글자 수 제한 오류를 방지합니다.
- **깔끔한 알림 메시지**: 슬랙 메시지 내 URL의 썸네일 미리보기를 비활성화하여 깔끔한 메시지를 제공합니다.
